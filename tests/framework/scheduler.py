...
processes to spawn).",
    ...
    def pytest_runtestloop(self, session):
        ...
        self._run_items(found_suite.get_runnable_items(session.items))
    ...
    def wait_for_worker_to_die(self, workers):
        ...
        If one of the workers dies, it returns it.
        """
        while True:
    ...
    def _run_items(self, items):
        ...
        while items_prio:
            ...
            if items_prio[0].can_run(workers):
                ...
                # Wait for a worker to die
                dead_worker = self.wait_for_worker_to_die(workers)
                del workers[dead_worker]
    ...
    def _worker_main(self, items, startup_delay=0):
        ...
        for item, nextitem in zip(
            self.session.items, self.session.items[1:] + [None]
        ):
